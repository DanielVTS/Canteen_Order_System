...
import cn.lingnan.view.popout.ClientOrderDialog2;
import cn.lingnan.DAO.ClientOrderDAO;
import cn.lingnan.DAO.OrderDAO;
import cn.lingnan.dto.Order;

public class StaffOrderView2 extends JPanel implements ActionListener{
    ...
    //上面
		...
		public StaffOrderView2(JFrame jFrame) {
		...
		private void initView() {
		    ...
		    Vector<Order> vm=md.findAllClientOrder2();
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (e.getSource() == searchBtn ) {			
				if(nameSearchTF.getText().isEmpty()) {
					Vector<Order> vm=sd.findAllClientOrder2();
					this.updateTable(vm);
					return;
				}else {
					Vector<Order> vm=sd.find2(Integer.parseInt(nameSearchTF.getText()));
					this.updateTable(vm);
					return;
				}							
		}
				if(addBtn==source) {
				ClientOrderDialog2 clientOrderDialog = new ClientOrderDialog2(jFrame,this);
				...
				old=sd.find2(id, name, status);
				
				if(old==null) {
					JOptionPane.showMessageDialog(this,"数据不存在,请刷新");
					return;
				}
				ClientOrderDialog2 clientOrderDialog = new ClientOrderDialog2(jFrame,this,old);
				clientOrderDialog.setVisible(true);
